...
To be able to use the marspylib functions in your Jupyter notebook or other Python code running environment, follow the procedure below. This package has no JVM/Fiji dependency, so any plain conda environment with `numpy`, `pandas`, and `matplotlib` will work.

**How to set it up**
1. Clone the repository from github so it is stored locally on your computer.
2. Open a terminal window, create/activate a conda environment (see `environment.yml`):
```
conda env create -f environment.yml
conda activate marspylib
```
...
To use the package, activate your conda environment in the terminal and launch a jupyter notebook. Open a notebook with that kernel and import the package similar to how you would also import other packages. For example:
...
yama.open("experiment.yama")
```

Check the file structure in the github repository to find whether a function is part of the main utilities folder, or whether a subdirectory (such as 'fret' or 'yama') needs to be specified.
